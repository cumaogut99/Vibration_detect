...
class MatplotlibCanvas(QWidget):
    ...
    def set_figure(self, fig) -> None:
        ...
        # When scrollable, give the canvas its natural size so QScrollArea
        # produces actual scroll bars instead of compressing the figure.
        if self._scroll is not None:
...
class WaterfallControlBar(QWidget):
    """
    Hz min / Hz max / RPM min / RPM max alanları + Yenile butonu.

    ``refresh_requested(freq_min, freq_max, rpm_min, rpm_max)`` sinyalini
    fırlatır. Boş kalan alan ``None`` olarak gelir (alıcı varsayılan
    aralığı uygulamalı).
    """
    ...
    def __init__(self, parent=None,
        ...
        # Enter tuşu basıldığında da tetikle
    ...
    def set_rpm_range(self, rpm_min: float, rpm_max: float) -> None:
        """RPM alanlarına run'ın aralığını placeholder olarak yaz."""
